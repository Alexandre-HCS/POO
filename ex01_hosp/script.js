...
class Funcionario {
    ...
    // Getter para acessar o CPF de forma controlada
    verCpf(visivel) {
      return visivel ? this.#cpf : "***.***.***-**"; // Esconde os números do CPF
    }

    // Getter para o CPF real (usado internamente)
    get cpfReal() {
      return this.#cpf;
    }
  }
  ...
  let cpfVisivel = false; // Controle de visibilidade do CPF
  ...
  atualizarTabela(); // Chama a função ao carregar a página